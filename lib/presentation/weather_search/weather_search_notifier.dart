...
///
/// 每次 [search] 都呼叫 [GetCityForecast]（=> Repository.fetchByCity =>
/// CWA API）。Notifier 自己不做任何 cache。
class WeatherSearchNotifier extends Notifier<WeatherViewState> {
  ...
  Future<void> search(String rawInput) async {
    ...
    state = WeatherLoading(trimmed);

    final result =
        await ref.read(getCityForecastProvider).call(rawInput);

    state = switch (result) {
      Ok(value: final forecast) => WeatherLoaded(forecast),
